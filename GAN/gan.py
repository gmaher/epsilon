...
def plot_gen(generator, noise_dim, n_ex=16, dim=(4,4), figsize=(10,10), fn='plot.png'):
...
# Set up Keras models
###########################################
def run(GNfilters, DNfilters, lrg, lrd, p, use_noise, EPS, Niter):
    NOISE_DIM = 100
    NBATCH = 20
    #GNfilters = 60
    #DNfilters = 20
    ...
    plt.legend()
    plt.savefig('{}_loss.png'.format(num_run), dpi=300)
    # plt.show()

    plt.figure()
    plot_gen(Generator, NOISE_DIM, fn='{}_gen.png'.format(num_run))

#########
# Do runs
#########
run_file = open('run_file.csv', 'a+', 0)
for num_run in range(0,400):

    GN = int(10**np.random.uniform(1,2.7))
    DN = int(10**np.random.uniform(1,2.7))
    lg = 10**(-np.random.uniform(2,5))
    ld = 10**(-np.random.uniform(2,5))
    p = np.random.uniform(0,1)
    un = np.random.randint(0,2)
    eps = 10**(-np.random.uniform(0,4))
    s = '{},{},{},{},{},{},{}\n'.format(GN,DN,lg,ld,p,un,eps)
    run_file.write(s)
    run_file.flush()
    run(GN,DN,lg,ld,p,un,eps,20000)
run_file.close()
